...
function CpanelUrl(cpanelUser: string, domain: string): string {
  // URL cPanel standard o2switch
...
export default function GrillePage() {
  ...
  // Trouver le domaine depuis les sites chargés
  const cpanelDomain = sites[0]?.domain ?? ""
  const cpanelUrl = selectedClass?.cpanelUser && cpanelDomain
    ? CpanelUrl(selectedClass.cpanelUser, cpanelDomain)
    : null
                          ...
                          <div className="mt-2 flex flex-col gap-1">
                            ...
                            {site.adminUrl && (
                              <a href={site.adminUrl} target="_blank" rel="noopener noreferrer"
                                onClick={e => e.stopPropagation()}
                                className="flex items-center gap-1 text-[10px] text-indigo-500 hover:underline truncate">
                                <ExternalLink size={9} /> Admin
                              </a>
                            )}
                            {site.adminUser && (
                              <div className="text-[9px] text-slate-400 mt-0.5" onClick={e => e.stopPropagation()}>
                                <span className="font-mono">{site.adminUser}</span>
                                {site.adminPass && (
                      ...
                      >
                        ...
                        {site.status === "ACTIVE" && (
                          <div className="mt-1 flex flex-col gap-0.5" onClick={e => e.stopPropagation()}>
                            {site.url && (
                              <a href={site.url} target="_blank" rel="noopener noreferrer"
                                className="text-[8px] text-blue-500 hover:underline truncate flex items-center gap-0.5">
                                <Globe size={7} /> Site
                              </a>
                            )}
                            {site.adminUrl && (
                              <a href={site.adminUrl} target="_blank" rel="noopener noreferrer"
                                className="text-[8px] text-indigo-500 hover:underline truncate flex items-center gap-0.5">
